...
fn main() {
    ...
    let exit_code = match cli.command {
        ...
        Commands::Config { allow_unfree } => {
            commands::config_execute(allow_unfree, &mut config)
        },
        Commands::GarbageCollect { delete_old, foreground } => {
        ...
        Commands::List => {
            commands::list_execute(&cli.passthrough_args, &config)
        },
    };
